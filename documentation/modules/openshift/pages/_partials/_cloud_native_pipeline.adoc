...
include::master@tekton-tutorial:ROOT:partial$namespace-prep.adoc[lines=13..15]
...
image::odc_deploy_app_from_git.png[]
...
By default the builder image will choosen `NodeJs` automatically based on the GitHub repository content as shown:

image::master@tekton-tutorial:openshift:odc_default_builder.png[]

But for the exercises of this chapter, we will build and deploy a Pipeline that will build a *Java* application. We can use Java applications under the folder https://github.com/redhat-scholars/tekton-tutorial/tree/master/apps[apps]. Let us change the builder to be Java, by clicking the btn:[Java] from the list of *Builder*:

.Use Java Builder
image::master@tekton-tutorial:openshift:odc_java_builder.png[]
...
----
<.> You can also set it to be `apps/greeter/java/quarkus`
...
Leave the *GITHUB_REPO_URL* and *GITHUB_REPO_REVISION* to be defaults

Set the menu:Workspace[Source] to be of type *PVC* and use *tekton-tutorial-sources* as PVC.

Set the menu:Workspace[maven-settings] to be of type *ConfigMap* and use *maven-settings* ConfigMap.
...
Click on the URL in the *Routes*, to access the application and see the response, which should be simple *hello* text.
