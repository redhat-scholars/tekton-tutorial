...
tekton-tutorial-greeter-image   image   url: localhost:5000/rhdevelopers/tekton-tutorial-greeter:v0.1.1
----
...
tkn pipeline start kn-svc-deploy \
 ...
 --resource="appImage=tekton-tutorial-greeter-image" \#<2>
...
<2> Bind the Pipeline `appImage` to pipeline resource `tekton-tutorial-greeter-image`
