...
.link:{github-repo}/{pipelines-repo}/kn-svc-deploy.yaml[svc-deploy.yaml]
...
The following resource defines a service account called `pipeline` in namespace `{tutorial-namespace}`, which will have needed permissions in the `{tutorial-namespace}` namespace to perform Tekton tasks.
