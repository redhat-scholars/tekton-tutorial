...
[source,bash,subs="+macros,+attributes"]
----
kubectl get svc -n {chapter-namespace} -leventlistener=gitea-webhook
----
...
----
NAME              TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)          AGE
#el-gitea-webhook   ClusterIP   10.97.199.207    <none>        8080/TCP         51s#
----
...
[source,bash,subs="+macros,+attributes"]
----
export EL_WEBHOOK_URL="$(kubectl get svc -n {chapter-namespace} el-gitea-webhook -o yaml \
...
[source,bash,subs="macros+,+attributes"]
----
oc expose svc -n {chapter-namespace} el-gitea-webhook --targetPort=http-listener
----
--
====
...
For the GitHub Triggers to work, we need to have the https://docs.gitea.io/en-us/webhooks/[Webhook] configured for the Git Repository `tekton-greeter` to point to the `el-gitea-webhook-ingress`:

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n {chapter-namespace} \
  -f link:{github-repo}/{triggers-repo}/gitea-webhook-taskrun.yaml[+$TUTORIAL_HOME+/{triggers-repo}/gitea-webhook-taskrun.yaml^]
----

[.console-output]
[source,bash]
----
taskrun.tekton.dev/webhook-gitea-bm8gv created
----
...
Using your favorite IDE clone the repo `tekton-greeter` from Gitea using the <<tkn-triggers-init-repo,clone url>>. 

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
git clone <your gitea tekton-greeter clone url>
----
...
+
--
- `springboot/src/main/java/com/redhat/developer/demos/GreetingController.java` to update the "Tekton 😺 rocks 🚀" to "Tekton 😺 Triggered" 
