...
- Git clone using `git-clone` task, the https://github.com/redhat-scholars/tekton-helloworld repo and build the apps/greeter/java/springboot application
...
spec:
  ...
  params: #<.>
    - name: url
      value: https://github.com/redhat-scholars/tekton-tutorial
    - name: revision
      value: master
  workspaces: #<.>
    - name: output
...
[clone] + test -z
[clone] + test -z
[clone] + /ko-app/git-init -url https://github.com/redhat-scholars/tekton-helloworld -revision master -refspec  -path /workspace/output/ '-sslVerify=true' '-submodules=true' -depth 1
[clone] {"level":"info","ts":1595778607.616774,"caller":"git/git.go:139","msg":"Successfully cloned https://github.com/redhat-scholars/tekton-helloworld @ 2e3336657cc1bbf22e3db183a517dcb0a62207b9 (grafted, HEAD, origin/master) in path /workspace/output/"}
...
If all went well the list directory should show the directory listing of https://github.com/redhat-scholars/tekton-helloworld.
