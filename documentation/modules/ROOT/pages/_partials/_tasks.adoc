...
1. Then the step two (`build-sources`) builds the application using https://apache.maven.org[Apache Maven]

2. The last step (`build-image-push`) uses the application artifacts generated by previous step, and builds the linux container image using https://buildah.io[buildah] and the https://github.com/redhat-scholars/tekton-tutorial/tree/master/apps/greeter/java/quarkus/Dockerfile.jvm[Dockerfile].
