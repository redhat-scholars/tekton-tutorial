...
[source,bash,subs="attributes+,+macros"]
----
watch minikube -p {tutorial-namespace} ssh -- sudo cat /etc/hosts
----
...
data:
  Corefile: |-
    .:53 {
        errors
        health
        rewrite name exampe.com registry.kube-system.svc.cluster.local
        rewrite name exampe.org registry.kube-system.svc.cluster.local
