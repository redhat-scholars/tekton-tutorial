...
* **serviceAccountName** - it is always recommended to have a service account associated with TaskRun, which can then be used to define fine grained roles.
...
In this example we make the Task's menu:spec[inputs>resources>source] to refer to pipeline resource `git-source` via the **resourceRef**. Similarly menu:spec[outputs>resources>builtImage] to refer to pipeline resource `tekton-tutorial-greeter-image` via the **resourceRef**.
