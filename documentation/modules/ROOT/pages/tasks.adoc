...
Listing the tasks in the `customertask-demo` will not show any Tasks as shown in the command output below. 
...
=== List and Describe ClusterTask s
...
[NOTE]
=====
If you are on OpenShift you can expose the service to get the route url, like:
...
[NOTE]
=====
If you are on OpenShift use routes like:
...
Now try <<tekton-task-run,running>> task again. When you examine the TaskRun pods should notice `stepTemplate` added to all the step containers including `build-sources`.
