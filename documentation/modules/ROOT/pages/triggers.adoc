...
helloworld-app-deploy   15 hours ago   helloworld-app-dm4wl   2 hours ago   6 minutes   ---
----
...
----
NAME                                  READY   STATUS    RESTARTS   AGE
pod/gogs-854766b6d7-nsmsp             1/1     Running   1          64s
...
A gogs home page should something like:
...
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n {chapter-namespace} gogs-triggerbindings.yaml
----
...
You can check the what payload Gogs might send in the Gogs Webhook settings page. We will be creating the Gogs Webhook as part upcoming section.

[#tkn-triggers-eventlistener]
== Event Listener

Event Listener is primary interace for external sources send send events to trigger creation of Tekton resources that is defined as part of the TriggerTemplate
...
The eventlistener creates the following services:
...
Using your favorite IDE clone the repo `tekton-helloworld`. Edit the Java file `src/java/com/redhat/developers/GreetingResource.java` to update the "Hello World!" to "😺 Tekton Rocks!" and update the Test case `test/java/com/redhat/developers/GreetingResourceTest.java` to match "😺 Tekton Rocks!", commit and push to see the 😺 Pipeline running 🚀.
