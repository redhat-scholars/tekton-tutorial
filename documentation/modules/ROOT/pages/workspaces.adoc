...
All exercises of this chapter will be done a namesapace `workspace-demo`, lets create and switch to the `workspace-demo` namespace.

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
kubectl create ns workspace-demo && \
kubectl config set-context --current --namespace workspace-demo
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl apply -n workspace-demo \
...
[source,bash,subs="+macros,+attributes"]
----
kubectl apply -n workspace-demo -f $TUTORIAL_HOME/install/utils/nexus.yaml
----
...
[source,bash,subs="+macros,+attributes"]
----
watch kubectl -n workspace-demo get pods,svc
----
...
[source,bash,subs="+macros,+attributes"]
----
$(minikube service nexus -n workspace-demo)
----
...
[source,bash,subs="+macros,+attributes"]
----
oc expose svc -n workspace-demo nexus
----
=====
...
For us to mount the maven `settings.xml`, we will create ConfigMap holding the settings.xml shown in the below listing:

.link:{github-repo}/{workspaces-repo}/maven-settings.xml[setttings.xml]
[source,xml]
----
include::ROOT:example$maven-settings.xml[]
----

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n workspace-demo cm maven-settings \
...
[source,bash,subs="+macros,+attributes"]
----
kubectl get -n workspace-demo cm maven-settings -oyaml \
...
In OpenShift cluster the default storage class varies based on the underlying cloud platform.
...
Since the cluster as *default* Storage Class, just specifying the PersistenceVolumeClaim will  bind the PVC automatically to underlying storage. Create the `tekton-tutorial-sources` PVC:

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
kubectl apply -n workspace-demo -f link:{github-repo}/{workspaces-repo}/sources-pvc.yaml[sources-pvc.yaml]
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n workspace-demo -f git-clone-taskrun.yaml
----

Grab the taskrun id to moinitor the logs,

[.console-output]
[source,bash]
----
taskrun.tekton.dev/#git-clone-dpd7t# created
----

Check for the logs using:

[.console-input]
[source,bash]
----
tkn tr logs -f -a git-clone-dpd7t
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n workspace-demo -f list-dir-taskrun.yaml
----
...
Grab the TaskRun id from the output of the earlier command to watch the logs:

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
tkn tr logs -f -a list-directory-mbkvl
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n workspace-demo -f maven-taskrun.yaml 
----
...
Grab the id from the output above and use that to watch logs:

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
tkn tr logs -f -a maven-build-9cjws
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n workspace-demo -f link:{github-repo}/{workspaces-repo}/list-target-dir-taskrun.yaml[list-target-dir-taskrun.yaml]
----
...
[source,bash,subs="+macros,+attributes"]
----
tkn tr logs -f -a list-directory-xbzpq
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl apply -n workspace-demo -f helloworld-app-deploy.yaml
----
...
wget -q -O- \
  ...
  | yq w - -d0 metadata.namespace workspace-demo \
  | yq w - -d2 subjects.[0].namespace workspace-demo \
  | kubectl apply -f -
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl create -n workspace-demo -f helloworld-app-deploy-run.yaml
----
...
Grab the id from the previous output and use the same to watch the logs

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
tkn pr logs -f -a helloworld-app-7k9cw
----
...
[source,bash,subs="+macros,+attributes"]
----
kubectl get ksvc -n workspace-demo
----
...
helloworld   http://helloworld.workspace-demo.192.168.64.5.nip.io   helloworld-kffxd-1   helloworld-kffxd-1   True
----
...
[source,bash,subs="+macros,+attributes"]
----
KSVC_URL=$(kubectl get routes helloworld -o yaml | yq r - status.url)
----
...
[source,bash,subs="+macros,attributes+"]
----
kubectl delete ns workspace-demo
----
